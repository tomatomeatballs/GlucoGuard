...
def model_training_page():
    ...
    if st.session_state['uploaded_data_frames']:
        ...
        with col_b:
            generate_btn = st.button("Generate Training Dataset", type="primary", use_container_width=True)
        ...
        with log_container:
            ...
            try:
                ...
                st.caption(
                    ...
                    "full optimization ~10-15 min depending on hardware."
                )
...
def blood_sugar_prediction_page():
    ...
    # =======================================================
    #  Real AI prediction from an uploaded history
    # =======================================================
    st.subheader("Upload Glucose History for Prediction")
...
Format with clear headings and bullet points. Keep it practical and easy to understand."""
        ...
        system_prompt = (
            ...
            "- Hypoglycemia threshold: 3.9 mmol/L or below is DANGEROUSLY LOW. "
            "If ANY predicted value (15/30/50 min) is ≤ 3.9 mmol/L, you MUST alert this as the #1 priority. "
            "Recommendations in this scenario: eat fast-acting carbs IMMEDIATELY (juice, glucose tablets, candy). "
            "NEVER suggest exercise, walking, or any physical activity when glucose is low or trending below 4.0 — exercise LOWERS blood glucose further and could cause severe hypoglycemia.\n"
            "- Hyperglycemia threshold: above 13.9 mmol/L is HIGH. Only in this scenario may you suggest light exercise (walking) as a potential management option, with appropriate cautions.\n"
            "- Exercise is ONLY safe to recommend when glucose is stable between 5.0–10.0 mmol/L and trending predictably. Even then, remind users to monitor closely.\n"
            "- When in doubt between safety and politeness, prioritize safety. A overly cautious recommendation is always better than a dangerous one.\n"
            "\n"
