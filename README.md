...
**MS2:** The "Launch Training" button ran a fake 7-step progress bar using `st.progress()` and `time.sleep()`. It didn't touch `main.py` at all. Training had to be done separately in the terminal.
...
**MS2:** `main.py` was a standalone script you ran once. It used `glob` to find files, hardcoded `zim=1` for all horizons, output everything to local disk.

**MS3:**
- `main.py` is superseded by `VMD_NOA_BILSTM.py` — takes `--user-id` argument, reads/writes database, supports different `zim` values per horizon (3/6/10 for 15/30/50min)
